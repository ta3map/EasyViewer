...
Скрипт `load_zav_file.m` предназначен для загрузки ZAV файлов с сохранением всей функциональности из основного приложения signalViewerGUI. Он обеспечивает полную совместимость с оригинальным кодом и может использоваться как самостоятельно, так и в составе других проектов.
...
Скрипт использует следующие функции из signalViewerGUI:
...
- Полностью совместим с signalViewerGUI v1.12.04
...
1. Скрипт создает временные переменные, которые не конфликтуют с глобальными переменными signalViewerGUI
2. Все функции signalViewerGUI должны быть доступны в текущем пути MATLAB
