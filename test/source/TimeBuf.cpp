...
ierr TimeBuf::OpenFile(CString FileName)
{
	...
	//open file
	fd=0;
	fd = compat_open( FileName, O_RDONLY);
	...
	if( str.Find("%%BEGINHEADER") != -1 ) {
		...
		newPosition=compat_lseek(fd,position,SEEK_SET);		
		...
		if (FileHeader) {
			//do the actual read
			compat_lseek(fd,0,SEEK_SET);
			BytesRead = compat_read(fd,HeaderBuf,NlxHeaderLength);
	...
	*/
	...
	if (Buf == NULL || Buf == MAP_FAILED) {
		Amb("Error mapping file to memory");
		FileOpen = FALSE;
		compat_close(fd);
		return(NLX_RANGE);
	}
