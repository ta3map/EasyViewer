...
if ~exist('selectedUnit', 'var') || isempty(selectedUnit)
    ...
    hPrevBtn = uicontrol(signalFig, 'Style', 'pushbutton', 'String', '◀ Previous', ...
        'Position', coordsData.elements.prev_btn, 'Callback', @(~,~)shiftTimeSlope(-1), 'Tag', 'prev_btn');
    hNextBtn = uicontrol(signalFig, 'Style', 'pushbutton', 'String', 'Next ▶', ...
        'Position', coordsData.elements.next_btn, 'Callback', @(~,~)shiftTimeSlope(1), 'Tag', 'next_btn');
    ...
    hAutoMeasureBtn = uicontrol(signalFig, 'Style', 'pushbutton', 'String', 'Auto Measure All', ...
        'Position', coordsData.elements.auto_measure_btn, 'Callback', @autoMeasureAllTimeRanges, 'Tag', 'auto_measure_btn');
    
    % Кнопка добавления результата
    hAddBtn = uicontrol(signalFig, 'Style', 'pushbutton', 'String', 'Add', ...
        'Position', coordsData.elements.add_btn, 'Callback', @addResult, 'Tag', 'add_btn');
    
    % Кнопка удаления результата
    hRemoveBtn = uicontrol(signalFig, 'Style', 'pushbutton', 'String', 'Remove', ...
        'Position', coordsData.elements.remove_btn, 'Callback', @removeResult, 'Tag', 'remove_btn');
    
    % Кнопка замены результата
    hReplaceBtn = uicontrol(signalFig, 'Style', 'pushbutton', 'String', 'Replace', ...
        'Position', coordsData.elements.replace_btn, 'Callback', @replaceResult, 'Enable', 'off', 'Tag', 'replace_btn');
    
    % Кнопка просмотра среднего сигнала
    hMeanResultsBtn = uicontrol(signalFig, 'Style', 'pushbutton', 'String', 'Av. Trace', ...
        'Position', coordsData.elements.mean_results_btn, 'Callback', @toggleMeanResults, 'Enable', 'off', 'Tag', 'mean_results_btn');
    
    % Кнопка открытия файла
    uicontrol(signalFig, 'Style', 'pushbutton', 'String', 'Open File', ...
        'Position', coordsData.elements.open_file_btn, 'Callback', @openFile, 'Tag', 'open_file_btn');
    
    % Кнопка групповых настроек
    uicontrol(signalFig, 'Style', 'pushbutton', 'String', 'Settings', ...
        'Position', coordsData.elements.settings_btn, 'Callback', @openGroupSettingsEditor, 'Tag', 'settings_btn');
    
    
    % Кнопка загрузки результатов
    uicontrol(signalFig, 'Style', 'pushbutton', 'String', 'Load', ...
        'Position', coordsData.elements.load_btn, 'Callback', @loadResults, 'Tag', 'load_btn');
...
uicontrol(signalFig, 'Style', 'pushbutton', 'String', 'Save', ...
    'Position', coordsData.elements.save_btn, 'Callback', @saveResults, 'Tag', 'save_btn');

% Кнопка сохранения изображения
uicontrol(signalFig, 'Style', 'pushbutton', 'String', 'Save Image', ...
    'Position', coordsData.elements.save_image_btn, 'Callback', @saveImage, 'Tag', 'save_image_btn');
    ...
    plotPanel = uipanel('Parent', signalFig, ...
        'Position', [0.22, 0.15, 0.483, 0.75], ...
        'Tag', 'plot_container');
    
    % Создаем оси внутри панели с координатами из JSON
    main_axes_coords = coordsData.elements.main_axes;
    hPlotAxes = axes('Parent', plotPanel, ...
        'Position', main_axes_coords, ...  % Координаты из JSON
        'Tag', 'main_axes');
    
    % Кнопка зума в левом углу графика
    hZoomButton = uicontrol(signalFig, 'Style', 'pushbutton', 'String', 'Zoom', ...
        'Position', coordsData.elements.zoom_btn, 'Callback', @toggleZoom, 'Tag', 'zoom_btn');
    ...
    hAutoscaleButton = uicontrol(signalFig, 'Style', 'pushbutton', 'String', 'Autoscale', ...
        'Position', coordsData.elements.autoscale_btn, 'Callback', @applyAutoscale, 'Tag', 'autoscale_btn');

    % === Таблица текущих результатов ===
    % Разделитель таблицы текущих результатов
    uicontrol(signalFig, 'Style', 'text', 'Position', [1135, 500, 460, 20], ...
    ...
    uicontrol(signalFig, 'Style', 'text', 'Position', [1135, 90, 460, 20], ...
    ...
    uicontrol(signalFig, 'Style', 'pushbutton', 'String', 'Clear Table', ...
        'Position', coordsData.elements.clear_table_btn, 'Callback', @clearAllResults, 'Tag', 'clear_table_btn');

    % === Таблица средних значений ===
    % Разделитель таблицы средних значений
    uicontrol(signalFig, 'Style', 'text', 'Position', [1135, 5, 460, 20], ...
